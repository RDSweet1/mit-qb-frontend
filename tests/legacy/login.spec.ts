  ...
  test('should login and initialize app successfully', async ({ page }) => {
    ...
    try {
      ...
      if (await loginButton.isVisible({ timeout: 5000 }).catch(() => false)) {
        ...
        // Set up popup handler before clicking login
        const popupPromise = page.waitForEvent('popup', { timeout: 10000 });

        // Click the login button
        await loginButton.click();

        // Wait for the Microsoft login popup to open
        console.log('🔍 Waiting for Microsoft login popup...');
        const popup = await popupPromise;
        console.log('✅ Popup opened:', popup.url());

        await popup.screenshot({ path: 'C:\\Users\\david\\AppData\\Local\\Temp\\claude\\C--SourceCode-WeeklyTimeBillingQB\\1330ec84-5c3c-4cd6-b13f-cc80867baad3\\scratchpad\\microsoft-login-popup.png', fullPage: true });
        console.log('📸 Screenshot saved to scratchpad/microsoft-login-popup.png');

        // Pause for manual login
        console.log('\n⏸️  Test paused. Please complete the Microsoft login in the POPUP window.');
        console.log('   The test will automatically continue after you close the popup.\n');

        // Wait for popup to close (indicates login completed - up to 5 minutes for manual login)
        await popup.waitForEvent('close', { timeout: 300000 });
        console.log('✅ Popup closed - login should be complete');

        // Wait a moment for the main page to update with authentication
